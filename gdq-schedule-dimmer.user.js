...
(function() {
  ...
  function style_run(now, run) {
    if (now > run.end) {
      ...
      for (let row of [run.row1, run.row2]) {
        ...
        for (let prop of ['backgroundImage', 'backgroundAttachment', 'boxShadow']) {
          row.style[prop] = '';
        }
      }
    }
      ...
      let pct = String(Math.round((now - run.start) / run.duration * 1000) / 10) + '%';
  ...
  for (let cell of cells) {
    let match = cell.textContent.match(/^\s*(\d+)-(\d+)-(\d+)T(\d+):(\d+):(\d+)Z\s*$/);
    if (!match) {
      continue;
    }
    ...
    let duration = parseInt(match[1], 10) * 3600 + parseInt(match[2], 10) * 60 + parseInt(match[3], 10);
    duration *= 1000;
